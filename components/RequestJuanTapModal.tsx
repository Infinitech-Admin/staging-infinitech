...
interface RequestJuanTapModalProps {
  isOpen: boolean;
...
export default function RequestJuanTapModal({
  ...
  const [currentSocial, setCurrentSocial] = useState<SocialMedia>({
    platform: "",
    url: "",
  });
  ...
  const addSocialMedia = () => {
    ...
    if (!currentSocial.platform || !currentSocial.url) {
      setErrors((prev) => ({
        ...prev,
        social_media:
          "Please select a platform and enter a profile URL before adding",
      }));
      return;
    }
    ...
    try {
      ...
      if (error instanceof Error) {
        if (
          ...
          errorMessage =
            "Server timeout. The image may be too large or the server is busy. Please try with a smaller image or try again later.";
        } else if (
      ...
      <div className="relative w-full max-w-2xl max-h-[calc(100vh-2rem)] sm:max-h-[calc(100vh-3rem)] flex flex-col bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="bg-slate-900/50 backdrop-blur-sm border-b border-blue-500/20 flex-shrink-0">
          <div className="px-4 sm:px-6 py-4 flex items-center justify-between gap-3">
            ...
            <button
              ...
              onClick={handleClose}
              aria-label="Close"
              className="flex-shrink-0 w-9 h-9 flex items-center justify-center rounded-lg text-blue-200 hover:text-white hover:bg-white/10 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div ref={scrollContainerRef} className="overflow-y-auto flex-1">
          <main className="px-4 sm:px-6 py-8">
            {submitted ? (
              <div className="max-w-lg mx-auto bg-white/95 backdrop-blur shadow-2xl rounded-2xl p-10 sm:p-12 text-center">
                ...
                <button
                  type="button"
                  onClick={handleClose}
                  className="mt-8 inline-flex items-center justify-center px-6 py-2.5 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white font-semibold rounded-lg transition-all"
                >
                  Close
                </button>
              </div>
              ...
              <div className="max-w-2xl mx-auto">
                <div className="text-center mb-8">
                  <h2 className="text-2xl sm:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-500 mb-3">
                      ...
                      {formData.social_media.length > 0 && (
                        <div className="space-y-2 mt-3">
                          <p className="text-xs text-slate-600 font-medium">
                            Added ({formData.social_media.length}):
                          </p>
