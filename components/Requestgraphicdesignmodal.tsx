...
interface RequestGraphicDesignModalProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
}

interface GraphicDesignFormData {
  name: string;
  email: string;
  phone: string;
  businessName: string;
...
const initialForm: GraphicDesignFormData = {
  name: "",
  email: "",
  phone: "",
  businessName: "",
...
const RequestGraphicDesignModal = ({
  isOpen,
  onOpenChange,
}: RequestGraphicDesignModalProps) => {
  const [form, setForm] = useState<GraphicDesignFormData>(initialForm);
  ...
  const handleChange = (
    field: keyof Omit<GraphicDesignFormData, "designCategories">,
    value: string,
  ) => {
  ...
  const toggleCategory = (category: string) => {
    setForm((prev) => ({
      ...prev,
      designCategories: prev.designCategories.includes(category)
        ? prev.designCategories.filter((c) => c !== category)
        : [...prev.designCategories, category],
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    setErrorMsg(null);
    try {
      const res = await fetch("/api/graphic-design-requests", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...form,
          designCategories: form.designCategories.join(","),
        }),
      });
            ...
            <>
              <ModalBody className="py-10 text-center">
                ...
                <p className="text-gray-500 text-sm mt-1">
                  Our design team will reach out about your project shortly.
                </p>
              </ModalBody>
            ...
            <>
              <ModalHeader className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-accent/10 shrink-0">
                  <FaPaintBrush className="h-4 w-4 text-accent" />
                </div>
                <div>
              ...
              <ModalBody className="gap-3">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {DESIGN_CATEGORY_OPTIONS.map((category) => {
                      const selected = form.designCategories.includes(category);
                      return (
                        <button
                          key={category}
                          type="button"
                ...
                <Textarea
                  ...
                  placeholder="e.g. We need a new logo and social media templates for our skincare brand launch..."
                  value={form.details}
...
export default RequestGraphicDesignModal;
