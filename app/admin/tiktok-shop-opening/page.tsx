// 📁 Place this file at: app/admin/tiktok-shop/page.tsx
"use client";
...
interface TikTokShopRequest {
  id: number;
  name: string;
  email: string;
  phone: string | null;
  business_name: string | null;
  product_categories: string | null; // comma-separated
  details: string | null;
...
export default function TikTokShopAdminPage() {
  const router = useRouter();
  const [requests, setRequests] = useState<TikTokShopRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedRequest, setSelectedRequest] =
    useState<TikTokShopRequest | null>(null);
  ...
  const [emailRequest, setEmailRequest] = useState<TikTokShopRequest | null>(
    null,
  );
  ...
  const fetchRequests = async () => {
    try {
      const response = await fetch("/api/tiktok-shop-requests");
      ...
      let requestsData: TikTokShopRequest[] = [];
      ...
      if (requestsData.length === 0) {
        setMessage("No TikTok Shop requests found yet.");
      }
    } catch (error) {
  ...
  const updateStatus = async (
    ...
    try {
      const response = await fetch(`/api/tiktok-shop-requests/${request.id}`, {
        method: "PATCH",
  ...
  const deleteRequest = async (request: TikTokShopRequest) => {
  ...
  const generatePDF = async (request: TikTokShopRequest) => {
    ...
    try {
      ...
      const addHeader = () => {
        ...
        doc.text("TikTok Shop Opening Request", pageWidth / 2, 15, {
          align: "center",
        });
      ...
      const addFooter = (pageNum: number, totalPages: number) => {
        ...
        doc.setFontSize(8);
        doc.setFont("helvetica", "bold");
        doc.text("TikTok Shop Requests", pageWidth / 2, pageHeight - 12, {
          align: "center",
        });
      ...
      addSectionHeader("SHOP DETAILS");
      addTableRow(
        "Product Categories",
        request.product_categories
          ? request.product_categories.split(",").join(", ")
          : "N/A",
        false,
      );
      addMultilineRow("Goals & Products", request.details || "N/A", true);
      ...
      doc.save(
        `tiktok-shop-${request.id}-${request.name.replace(/\s+/g, "-").toLowerCase()}.pdf`,
      );
  ...
  const openEmailDialog = (request: TikTokShopRequest) => {
    setEmailRequest(request);
    setEmailSubject(
      `Your TikTok Shop Opening Request - ${request.business_name || request.name}`,
    );
    setEmailMessage(`Dear ${request.name},

Thank you for requesting to open a TikTok Shop${
      request.business_name ? ` for ${request.business_name}` : ""
    }.

We've received your submission${
      request.product_categories
        ? ` covering: ${request.product_categories.split(",").join(", ")}`
        : ""
    }. Our team will review your details and follow up shortly to get your shop live.
...
The Team`);
  ...
  const sendRequestEmail = async () => {
    ...
    try {
      const response = await fetch("/api/send-tiktok-shop-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const filteredRequests = Array.isArray(requests)
    ? requests.filter((request) => {
        ...
        return (
          ...
          (request.product_categories?.toLowerCase() || "").includes(q)
        );
      })
    : [];
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 bg-gradient-to-br from-pink-500 to-slate-900 rounded-xl flex items-center justify-center">
              <Store className="h-5 w-5 text-white" />
            </div>
            <div>
              <h1 className="text-4xl font-bold text-slate-900 dark:text-white">
                TikTok Shop Requests
              </h1>
              <p className="text-muted-foreground">
                View and manage all TikTok Shop opening requests
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="mb-6 border-2">
          <CardContent className="p-6">
            <div className="relative">
              ...
              <Input
                placeholder="Search by name, email, business, category, or phone..."
                value={searchQuery}
                onChange={(e) => {
        ...
        <Card className="border-2 overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-pink-50 to-slate-100 dark:from-pink-950 dark:to-slate-900 border-b-2">
              ...
              <Table>
                ...
                <TableBody>
                  {paginatedRequests.map((request) => {
                    const categories = request.product_categories
                      ? request.product_categories.split(",").filter(Boolean)
                      : [];

                    return (
                      <TableRow
                        key={request.id}
                        className="hover:bg-pink-50/50 dark:hover:bg-pink-900/20 border-b"
                      >
                        ...
                        <TableCell>
                          <Select
                            ...
                            onValueChange={(value) =>
                              updateStatus(request, value as RequestStatus)
                            }
                            disabled={updatingStatusId === request.id}
                          >
                            <SelectTrigger className="h-7 w-[110px] border-0 p-0 focus:ring-0 [&>svg]:hidden">
                            ...
                            <Dialog>
                              <DialogTrigger asChild>
                                <Button
                                  ...
                                  className="border-2 border-pink-200 hover:bg-pink-50 hover:text-pink-700 dark:border-pink-800 dark:hover:bg-pink-900/20"
                                >
                              ...
                              {selectedRequest?.id === request.id && (
                                <DialogContent className="max-w-[92vw] sm:max-w-md max-h-[85vh] overflow-y-auto border-2 p-0 gap-0">
                                  <div className="space-y-0">
                                    <div className="bg-gradient-to-r from-pink-500 to-slate-900 px-4 py-3 text-white">
