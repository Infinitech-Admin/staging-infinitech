// 📁 Place this file at: app/admin/paid-ads/page.tsx
"use client";
...
export default function PaidAdsAdminPage() {
  const router = useRouter();
  const [requests, setRequests] = useState<PaidAdsRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedRequest, setSelectedRequest] = useState<PaidAdsRequest | null>(
    null,
  );
  ...
  const [emailRequest, setEmailRequest] = useState<PaidAdsRequest | null>(null);
  ...
  const fetchRequests = async () => {
    try {
      const response = await fetch("/api/paid-ads-requests");
      ...
      let requestsData: PaidAdsRequest[] = [];
      ...
      if (requestsData.length === 0) {
        setMessage("No paid ads requests found yet.");
      }
    } catch (error) {
  ...
  const markAsRead = async (request: PaidAdsRequest) => {
    try {
      const response = await fetch(`/api/paid-ads-requests/${request.id}`, {
        method: "PATCH",
  ...
  const deleteRequest = async (request: PaidAdsRequest) => {
  ...
  const generatePDF = async (request: PaidAdsRequest) => {
    ...
    try {
      ...
      const addHeader = () => {
        ...
        doc.text("Paid Ads Request", pageWidth / 2, 15, {
          align: "center",
        });
      ...
      const addFooter = (pageNum: number, totalPages: number) => {
        ...
        doc.setFontSize(8);
        doc.setFont("helvetica", "bold");
        doc.text("Paid Ads Requests", pageWidth / 2, pageHeight - 12, {
          align: "center",
        });
      ...
      addSectionHeader("CAMPAIGN DETAILS");
      addTableRow(
        "Ad Platforms",
        request.ad_platforms
          ? request.ad_platforms.split(",").join(", ")
          : "N/A",
        false,
      );
      addTableRow("Monthly Budget", request.monthly_budget || "N/A", true);
      addMultilineRow("Campaign Goals", request.details || "N/A", false);
      ...
      doc.save(
        `paid-ads-${request.id}-${request.name.replace(/\s+/g, "-").toLowerCase()}.pdf`,
      );
  ...
  const openEmailDialog = (request: PaidAdsRequest) => {
    setEmailRequest(request);
    setEmailSubject(
      `Your Paid Ads Request - ${request.business_name || request.name}`,
    );
    setEmailMessage(`Dear ${request.name},

Thank you for your interest in running paid ads${
      request.business_name ? ` for ${request.business_name}` : ""
    }.

We've received your submission${
      request.ad_platforms
        ? ` covering: ${request.ad_platforms.split(",").join(", ")}`
        : ""
    }${
      request.monthly_budget
        ? ` with an estimated monthly budget of ${request.monthly_budget}`
        : ""
    }. Our ads team will review your goals and follow up shortly with next steps.
...
The Team`);
  ...
  const sendRequestEmail = async () => {
    ...
    try {
      const response = await fetch("/api/send-paid-ads-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const filteredRequests = Array.isArray(requests)
    ? requests.filter((request) => {
        ...
        return (
          ...
          (request.ad_platforms?.toLowerCase() || "").includes(q) ||
          (request.monthly_budget?.toLowerCase() || "").includes(q)
        );
      })
    : [];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                View and manage all paid ads requests
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="mb-6 border-2">
          <CardContent className="p-6">
            <div className="relative">
              ...
              <Input
                placeholder="Search by name, email, business, phone, platform, or budget..."
                value={searchQuery}
                onChange={(e) => {
        ...
        <Card className="border-2 overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950 dark:to-purple-950 border-b-2">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-100 dark:bg-slate-800">
                    ...
                    <TableHead className="font-bold">Platforms</TableHead>
                ...
                <TableBody>
                  {paginatedRequests.map((request) => (
                    <TableRow
                      key={request.id}
                      className="hover:bg-blue-50/50 dark:hover:bg-blue-900/20 border-b"
                    >
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2 justify-center">
                                ...
                                <div className="space-y-0">
                                  ...
                                  <div className="px-4 py-3 space-y-3">
                                    <div className="space-y-1.5">
                                      ...
                                      {request.ad_platforms && (
                                        <div className="flex items-center gap-1.5 p-2 bg-slate-50 dark:bg-slate-900 rounded-md text-xs">
                                          <Megaphone className="h-3.5 w-3.5 flex-shrink-0 text-slate-500" />
                                          {request.ad_platforms
                                            .split(",")
                                            .join(", ")}
                                        </div>
                                      ...
                                      {request.monthly_budget && (
                                        <div className="flex items-center gap-1.5 p-2 bg-slate-50 dark:bg-slate-900 rounded-md text-xs">
                                          <Wallet className="h-3.5 w-3.5 flex-shrink-0 text-slate-500" />
                                          {request.monthly_budget}
                                        </div>
                                      )}
                                    </div>
                          ...
                          <Button
                            ...
                            className="border-2 border-purple-200 hover:bg-purple-50 hover:text-purple-700 dark:border-purple-800 dark:hover:bg-purple-950/20"
                          >
