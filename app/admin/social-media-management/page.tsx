// 📁 Place this file at: app/admin/social-media/page.tsx
"use client";
...
export default function SocialMediaAdminPage() {
  ...
  const [selectedRequest, setSelectedRequest] =
    useState<SocialMediaRequest | null>(null);
  ...
  const [emailRequest, setEmailRequest] = useState<SocialMediaRequest | null>(
    null,
  );
  ...
  const fetchRequests = async () => {
    try {
      const response = await fetch("/api/social-media-requests");
      ...
      let requestsData: SocialMediaRequest[] = [];
      ...
      if (requestsData.length === 0) {
        setMessage("No social media management requests found yet.");
      }
    } catch (error) {
  ...
  const markAsRead = async (request: SocialMediaRequest) => {
    try {
      const response = await fetch(`/api/social-media-requests/${request.id}`, {
        method: "PATCH",
  ...
  const deleteRequest = async (request: SocialMediaRequest) => {
  ...
  const generatePDF = async (request: SocialMediaRequest) => {
    ...
    try {
      ...
      const addHeader = () => {
        ...
        doc.text("Social Media Management Request", pageWidth / 2, 15, {
          align: "center",
        });
      ...
      const addFooter = (pageNum: number, totalPages: number) => {
        ...
        doc.setFont("helvetica", "bold");
        doc.text(
          "Social Media Management Requests",
          pageWidth / 2,
          pageHeight - 12,
          { align: "center" },
        );
      ...
      addTableRow("Company", request.company || "N/A", true);

      addSectionHeader("REQUEST DETAILS");
      addTableRow(
        "Platforms",
        request.platforms ? request.platforms.split(",").join(", ") : "N/A",
        false,
      );
      addMultilineRow("Goals / Presence", request.details || "N/A", true);
      ...
      doc.save(
        `social-media-${request.id}-${request.name.replace(/\s+/g, "-").toLowerCase()}.pdf`,
      );
  ...
  const openEmailDialog = (request: SocialMediaRequest) => {
    setEmailRequest(request);
    setEmailSubject(
      `Your Social Media Management Request - ${request.company || request.name}`,
    );
    setEmailMessage(`Dear ${request.name},

Thank you for your interest in social media management${
      request.company ? ` for ${request.company}` : ""
    }.

We've received your submission${
      request.platforms
        ? ` covering: ${request.platforms.split(",").join(", ")}`
        : ""
    }. Our team will review your goals and follow up shortly with a proposed content plan.
...
The Team`);
  ...
  const sendRequestEmail = async () => {
    ...
    try {
      const response = await fetch("/api/send-social-media-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const filteredRequests = Array.isArray(requests)
    ? requests.filter((request) => {
        ...
        return (
          ...
          (request.platforms?.toLowerCase() || "").includes(q)
        );
      })
    : [];
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                View and manage all social media management requests
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="mb-6 border-2">
          <CardContent className="p-6">
            <div className="relative">
              ...
              <Input
                placeholder="Search by name, email, company, phone, or platform..."
                value={searchQuery}
                onChange={(e) => {
                                ...
                                <div className="space-y-0">
                                  ...
                                  <div className="px-4 py-3 space-y-3">
                                    <div className="space-y-1.5">
                                      ...
                                      {request.platforms && (
                                        <div className="flex items-center gap-1.5 p-2 bg-slate-50 dark:bg-slate-900 rounded-md text-xs">
                                          <Hash className="h-3.5 w-3.5 flex-shrink-0 text-slate-500" />
                                          {request.platforms
                                            .split(",")
                                            .join(", ")}
                                        </div>
